...
int main(int argc, char* argv[])
{
	...
	extApi_sleepMs(10000); // wait 20 seconds before connecting to ports, gives v-rep time to start up.
...
//		client->ea->savePopFitness(i + 1, client->ea->popFitness);
		...
		if (client->settings->verbose) {
			std::cout << "Just saved settings <right after evaluate pop>" << endl;
		}
